...
@Component({
    ...
    template: `
        <div
            #gridCanvas
            class="m-grid"
            [ngStyle]="{ width: params.canvasWidth + 'px' }"
        >
                ...
                <ng-container *ngFor="let row of viewModel.rows">
                    <div #rowRef class="m-grid__row">
                        <ng-container *ngFor="let child of row.children">
                            <div
                                #itemRef
                                class="m-grid__item {{ child.styleClass }}"
...
export class MGridComponent implements OnInit, AfterViewInit {
    ...
    setViewModel() {
        let previousLevel, previousParent, currentRow;
        ...
        Object.keys(this.model.entities).forEach(key => {
            ...
            if (Array.isArray(currentItem.children)) {
                currentItem.children = currentItem.children.map(
                    (id: string) => this.model.entities[id]
                );
            }
    ...
    public ngAfterViewInit() {
        ...
        this.gridCanvasElement.addEventListener(
            "mousewheel",
            this.onMouseWheel
        );
        this.gridCanvasElement.addEventListener(
        ...
        for (
            let i = 0;
            i < Object.keys(this.modelEntitiesByLevel).length;
            i++
        ) {
    ...
    private shiftElementsInRow(rowIndex, row): void {
        if (rowIndex === 0) {
            ...
            this.itemElementMap[gridItem.id].style.left =
                gridItem.centralAxes - this.params.itemWidth / 2 + "px";
        } else {
            ...
            this.modelEntitiesByLevel[rowIndex].forEach((gridItem, index) => {
                ...
                const gridItemsWithSameParentLength = this.getSameParentItemsLength(
                    this.modelEntitiesByLevel[rowIndex],
                    gridItem
                );
                ...
                const position =
                    ...
                    (gridItemsWithSameParentLength * this.params.itemWidth) /
                        2 +
                    ...
                    (index - childrenOfPreviousParentInLevelNumber) *
    ...
    private getShiftByChildren(gridItem: GridItemModel): number {
        return (
            this.getCountedShiftByChildrenOneSide(gridItem) *
            this.getGridItemShiftSign(gridItem)
        );
    }
    ...
    private getCountedShiftByChildrenOneSide(gridItem: GridItemModel) {
        if (gridItem.children && gridItem.children.length) {
            return (
                (gridItem.children.length > 1
                    ? (gridItem.children.length - 1) / 2
                    : 1) * this.params.itemWidth
            );
        }
    ...
    //  shift previous siblings according to current grid item children
    private shiftPreviousSiblings(
        level: GridItemModel[],
        gridItem: GridItemModel
    ): void {
        const shiftSign = this.getGridItemShiftSign(gridItem);

        const gridItemChildrenLength = gridItem.children
            ? gridItem.children.length
            : 0;
        const previousSiblingsOnLevel = this.getPreviousSiblingsOnLevel(
            level,
            gridItem
        );
            ...
            if (shiftSign < 0) {
                previousSiblingsOnLevel.forEach((item: GridItemModel) => {
                    ...
                    item.centralAxes +=
                        ...
                        this.getCountedShiftByChildrenOneSide(gridItem) *
                        2;
                    this.itemElementMap[item.id].style.left =
                        item.centralAxes - this.params.itemWidth / 2 + "px";
                });
            } else {
                ...
                this.getPreviousSiblingsOnLevelBeforeCenterAxes(
                    level,
                    gridItem
                ).forEach(item => {
                    previousRightSiblingsChildrenLength += this.getCountedShiftByChildrenOneSide(
                        item
                    );
                });
                ...
                if (previousRightSiblingsChildrenLength) {
                    ...
                    // count all
                    gridItem.centralAxes +=
                        previousRightSiblingsChildrenLength +
                        this.getCountedShiftByChildrenOneSide(gridItem) +
                        this.params.itemWidth;
                    this.itemElementMap[gridItem.id].style.left =
                        gridItem.centralAxes - this.params.itemWidth / 2 + "px";
    ...
    private getPreviousSiblingsOnLevel(
        level,
        gridItem: GridItemModel
    ): GridItemModel[] {
    ...
    private getPreviousSiblingsOnLevelBeforeCenterAxes(
        level,
        gridItem: GridItemModel
    ): GridItemModel[] {
        return level.filter(item => {
            if (gridItem.id !== item.id) {
                return (
                    item.centralAxes >= gridItem.centralAxes &&
                    item.centralAxes > this.mainAxesCoords
                );
            }
        });
    }
    ...
    private convertQueryListToElementMap(
        arr: QueryList<ElementRef>
    ...
    private convertQueryListToElementList(
        arr: QueryList<ElementRef>
    ): HTMLElement[] {
